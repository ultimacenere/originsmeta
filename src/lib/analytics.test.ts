...
describe("catalogo degli eventi", () => {
  ...
  test("i quattro eventi già online dal 25/09/2026 (a9400e8) tengono il loro nome, senza doppioni con un altro nome", () => {
    for (const name of ["sign_up", "deck_created", "deck_published", "tierlist_created"]) assert.ok(A.isEventName(name), name);
    for (const name of ["deck_publish", "deck_save_private", "tier_list_save"]) assert.ok(!A.isEventName(name), `${name}: stessa azione di un evento già online`);
  });
...
describe("nel browser", () => {
  ...
  test("traffico interno: solo il codice giusto accende il flag; ?staff=off lo spegne; le pagine viste di Vercel lo seguono", async () => {
    ...
    A.trackEvent("deck_published", { locale: "it", legendary: "merlin", source: "builder" });
  ...
  test("consenso ridato nella stessa pagina (ricaricamento rimandato): GA4 si riaccende", () => {
    ...
    A.trackEvent("tierlist_created", { locale: "it", kind: "cards" });
    assert.deepEqual(gaEvents().at(-1), ["event", "tierlist_created", { locale: "it", kind: "cards", lang: "it" }]);
