...
describe("eventi e link della scheda", () => {
  ...
  test("il clic su riproduci del lettore a clic della scheda è un video, quello di una guida no", () => {
    assert.equal(statKindForEvent("video_play", { provider: "youtube", placement: "deck_page" }), "video");
    assert.equal(statKindForEvent("video_play", { provider: "twitch", placement: "deck_page" }), "video");
    assert.equal(statKindForEvent("video_play", { provider: "youtube", placement: "guide" }), null);
  });
