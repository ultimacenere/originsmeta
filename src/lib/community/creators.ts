...
import { CREATOR_BADGES, cleanContentLangs, mainChannels, parseStoredLinks, twitchLogin, type ContentLang, type ProfileLink } from "./profileLinks";
 ...
 * "nessun dato" una volta per istanza, come per la colonna `translations` in queries.ts, e il sito resta com'era:
 * /creators vuota e noindex, niente icone accanto ai nomi, profili senza bio.
 */
...
function missingColumns(error: ReadError): boolean {
  ...
  if (missing && showcaseColumns) {
    showcaseColumns = false;
    console.error("[community] mancano le colonne del profilo pubblico (bio, links, content_langs): va applicato supabase/creator-CREATOR.sql");
  }
  return missing;
}
...
export async function getProfileShowcase(profileId: string): Promise<Showcase | null> {
  ...
  if (!client || !showcaseColumns) return null;
 ...
 * cancellerebbe canali che ci sono).
 */
...
export async function listCreators(): Promise<CreatorProfile[]> {
  const client = supabasePublic();
  if (!client || !showcaseColumns) return [];
...
/** Le stesse colonne di TOURNAMENT_SELECT in tournament/queries.ts (non esportata), per `TournamentCard`. */
const ORGANIZED_SELECT = `id, slug, tag, organizer, name, cover_url, description, rules, lang, starts_at, size, format, deck_mode, conquest_decks, conquest_min_different, best_of, discord_url, status, listed, report, visibility, created_at, updated_at, ${ORGANIZER}, players:tournament_players(count)`;

/**
 * I tornei PUBBLICI organizzati da un iscritto, dal più recente (vetrina del creator su /u). Mai i privati: il filtro
 * è esplicito, anche se la policy `can_view_tournament` li nasconderebbe comunque al client anonimo. Niente annullati.
 */
