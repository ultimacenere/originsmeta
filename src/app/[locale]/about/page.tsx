...
export default async function AboutPage({ params }: { params: LocaleParams }) {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6">
      ...
      <h1 className="mt-2 text-4xl font-extrabold text-chalk sm:text-5xl">{d.about.title}</h1>
      ...
      <article className="card-ivory mt-8 space-y-4 p-6 text-lg leading-relaxed text-ink sm:p-10">
        <p>{d.about.p1}</p>
        <p>{d.about.p2}</p>
        <p>{d.about.p3}</p>
        ...
        <p>
          {d.about.contactText}{" "}
          <a className="text-crimson-deep underline" href={`mailto:${contactEmail}`}>
            {contactEmail}
          </a>
        </p>
        ...
        <p className="text-base text-ink-muted">{d.about.disclaimer}</p>
      </article>
    </div>
  );
}
