    ...
    <div>
      ...
      {list.length === 0 ? (
        ...
        <ul className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {list.map((c) => (
            <li key={c.slug}>
              <Link href={c.href} className="card-ivory card-ivory-hover flex h-full gap-4 p-4">
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="mt-1 block truncate font-display text-lg font-bold leading-tight text-ink">{c.name}</span>
                  <span className="mt-2 flex flex-wrap items-center gap-2 text-sm">
                    ...
                    {c.legendary ? <span className="stat-pill bg-gold/40 text-ink">{labels.legendary}</span> : null}
