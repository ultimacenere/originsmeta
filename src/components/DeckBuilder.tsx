      ...
      <section className="card-ivory p-5 sm:p-6">
            ...
            <div className="flex gap-1">
              {[0, 1, 2].map((i) => (
                <button
                  ...
                  className={`rounded-lg px-3 py-1 font-display text-xs font-bold ${active === i ? "bg-mint text-ink" : "border border-sky text-ink-muted"}`}
                >
        ...
        <label className="mt-4 block">
          ...
          <input
            ...
            className="mt-1 w-full rounded-lg border border-sky bg-ivory px-3 py-2 text-ink"
          />
        </label>
        ...
        {deck.legendary && slotCard(deck.legendary) ? (
          ...
          <p className="mt-1 rounded-lg border border-dashed border-sky px-3 py-3 text-sm text-ink-muted">{labels.pickLegendary}</p>
        )}
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {complete ? (
            ...
            <span className="btn cursor-not-allowed border border-sky text-xs text-ink-muted" title={labels.publishHint} aria-disabled="true">
              {labels.publish}
            </span>
          )}
          ...
          <a
            className="btn border border-sky text-xs text-ink"
          ...
          <button
            type="button"
            className="btn border border-sky text-xs text-ink hover:text-crimson-deep"
        ...
        <textarea
          ...
          rows={3}
          className="mt-2 w-full rounded-lg border border-sky bg-ivory px-3 py-2 font-mono text-xs text-ink"
        />
              ...
              {unknownKeys.map((k) => (
                <li key={k} className="flex items-center gap-2">
                  ...
                  <select
                    ...
                    className="flex-1 rounded border border-sky bg-ivory px-2 py-1 text-xs"
                  >
                    <option value="">—</option>
                    {pool.map((c) => (
          ...
          <div className="mt-6 rounded-lg border border-sky p-4">
            ...
            <label className="mt-2 flex items-center gap-2 text-xs text-ink-muted">
              ...
              <input
                ...
                className="w-16 rounded border border-sky bg-ivory px-2 py-1 font-mono text-ink"
              />
            </label>
...
function DeckRow({ card, copies, onRemove, removeLabel }: { card: BuilderCard; copies: number; onRemove: () => void; removeLabel: string }) {
  return (
    <div className="flex items-center gap-2 rounded-lg border border-sky bg-ivory-2/60 px-2 py-1.5">
      ...
      <button type="button" onClick={onRemove} className="stat-pill border border-sky text-[10px] text-ink hover:bg-crimson hover:text-ivory" aria-label={`${removeLabel} ${card.name}`}>
        ✕
      </button>
    </div>
  );
