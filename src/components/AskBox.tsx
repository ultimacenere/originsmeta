...
export function AskBox({ locale, labels, suggerimenti, hrefPrefix }: { locale: string; labels: AskLabels; suggerimenti: string[]; hrefPrefix: string }) {
  ...
  async function chiedi(testo: string) {
    ...
    try {
      ...
      if (!r.ok) {
        ...
        setErrore(labels.errors[(codice as keyof typeof labels.errors) in labels.errors ? (codice as keyof typeof labels.errors) : "generico"]);
        return;
      }
      const dati = (await r.json()) as { risposta: string; fonti: Fonte[] };
      setRisposta({ testo: dati.risposta, fonti: dati.fonti ?? [] });
    } catch {
      setErrore(labels.errors.generico);
    } finally {
      setStato("fermo");
    }
  }
    ...
    <div className="card-night p-5 sm:p-6">
      ...
      >
        ...
        <div className="mt-3 flex flex-wrap items-center gap-3">
          <button type="submit" disabled={stato === "attesa"} className="btn btn-mint disabled:opacity-60">
      ...
      {errore ? <p className="mt-4 rounded-lg border border-crimson bg-crimson/10 px-3 py-2 text-sm text-chalk">{errore}</p> : null}
