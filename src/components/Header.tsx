      ...
      <div className="mx-auto flex max-w-7xl items-center gap-3 px-4 py-3 sm:px-6">
        ...
        <nav className="ml-4 hidden items-center gap-0.5 xl:flex" aria-label="Main">
          {items.map((it) => (
            <Link
              ...
              className="rounded-full px-3 py-2 font-display text-[0.95rem] font-medium text-chalk-muted transition hover:bg-felt-soft hover:text-mint"
            >
              {it.label}
            </Link>
          ))}
        ...
        <form action={href(locale, "/cards")} method="get" role="search" className="ml-auto hidden items-center md:flex">
          ...
          <input
            ...
            className="w-44 rounded-l-full border border-felt-line bg-felt px-4 py-2 text-sm text-chalk placeholder:text-chalk-muted/70 focus:border-mint lg:w-56"
          />
