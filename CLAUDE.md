...
- Tasti verso Steam e Discord: sempre `SteamButton`/`DiscordButton` (colori e font ufficiali, richiesta esplicita di Pierluigi). Niente elementi bianchi né neri pieni (richiesta del 15/09/2026): le schede sono pannelli blu notte (`card-night`, token `night`/`night-2`/`night-3`) con cornice celeste piena da 3 px (`sky` #3fc4e8, il cristallo del mana; niente righette sottili), titoli e nomi in menta come il titolo di MetaShift (`text-mint`), testi in grigio chiaro (`pale`/`chalk`, versioni `-muted` per il secondario). `ink` resta solo per il testo scuro sui fondi pieni menta e oro.
...
Sfondo viola inchiostro (`felt`), carte blu notte (`night`) con cornici celesti piene (`sky`), titoli in menta, testi grigio chiaro (`pale`, `chalk`), accento dominante menta (`mint`) per bottoni e link, giallo citrino solo per leggendarie ed evento evidenziato (`gold`), magenta per nerf e avvisi (`crimson`). Font: Unbounded (display), Manrope (testo), JetBrains Mono (numeri).
