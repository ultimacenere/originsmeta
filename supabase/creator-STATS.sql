...
--   (src/components/DeckStatsBeacon.tsx): una volta per sessione del browser, per mazzo e per tipo, dopo qualche
--   secondo di pagina visibile per la visita; niente bot, niente browser dello staff, niente autore sul proprio mazzo.
--   Sono stime: chi volesse gonfiarle con uno script può farlo fino al tetto giornaliero qui sotto.
...
-- Tetto per contatore, mazzo e giorno: ferma chi gonfiasse i numeri con uno script (molto sopra il traffico vero).
...
begin
  ...
  on conflict (deck_id, day) do update set
    views = least(s.views + excluded.views, cap),
    code_copies = least(s.code_copies + excluded.code_copies, cap),
    link_clicks = least(s.link_clicks + excluded.link_clicks, cap),
    video_plays = least(s.video_plays + excluded.video_plays, cap);
end $$;
