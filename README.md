...
`/tier-list/community` non si aggiorna a mano: è la media delle tier list che gli iscritti salvano da `/tier-list/create` con il tasto "Salva nel profilo". Ogni fascia vale un punteggio (S=5 … D=1), la vista `tier_card_scores` di `supabase/schema.sql` fa la media per carta e `src/lib/community/tierlists.ts` la riporta in una fascia (`tierFromScore`) tenendo solo le carte con almeno `MIN_TIER_VOTES` voti. Una tier list per utente e per tipo (Leggendarie, carte base): salvarne un'altra sostituisce la propria. La tendina fra le tre pagine è `src/components/TierListNav.tsx`.
